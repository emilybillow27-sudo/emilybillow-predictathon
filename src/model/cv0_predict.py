...
if len(sys.argv) < 2:
    raise SystemExit("Usage: python src/cv0_predict.py <TRIAL>")
...
# Output directory
out_dir = f"{ROOT}/results/cv0_predictions"
...
# ---------------------------------------------------------
# Identify focal-trial accessions (even if phenotype is missing)
# ---------------------------------------------------------
...
# All accessions in the trial
all_trial_lines = sorted(pheno_trial["id_for_grm"].unique())

# Split into genotyped vs ungenotyped
...
# ---------------------------------------------------------
# CV0 training set: all other trials WITH phenotypes
# ---------------------------------------------------------
train = pheno[(pheno["trial"] != trial) & (~pheno["value"].isna())].copy()
train = train[train["id_for_grm"].isin(grm_set)]

if train.empty:
    # fallback: predict training mean for all lines
    ...
    print(f"✓ CV0 predictions saved for {trial} (no training data; zero used)")
    sys.exit()

# Unique training lines
...
print(f"✓ CV0 predictions saved for {trial} (including fallback for ungenotyped lines)")